...
int UserUpdate(pthread_t selfId, ThreadParameter *threadParam, char *recvBuf, char *sendBuf){ 
  ...
  char comm[BUFSIZE];
  int sendLen;
  char user_id[IDSIZE], role[ROLESIZE];
  char name[NAMESIZE], password[PASSSIZE]; 

  sscanf(recvBuf, "%s %s %s %s %[^\n]", comm, user_id, password, role, name);
  ...
  snprintf(sql, SQLSIZE,
           "UPDATE users SET password = '%s', role = '%s', name = '%s' "
           "WHERE user_id = '%s'",
           password, role, name, user_id);
  printf("[%s]\n", sql);
